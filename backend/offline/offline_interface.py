...
class OfflineInterface(object):
    ...
    def setup(self, time_series_db_index=0, clustering_db_index=0, serializing_db_index=0):
        time_series_db = self._data_model_interface.get_time_series_database(time_series_db_index)
        clustering_db = self._data_model_interface.get_clustering_database(clustering_db_index)
        serializing_db = self._data_model_interface.get_serialization_database(serializing_db_index)
    ...
    def transfer_time_series(self, catalog_name, source_database_index, destination_database_index=0):
        ...
        destination_db = self._data_model_interface.get_time_series_database(destination_database_index)
        batch_iterable = source_db.get_all(1)#todo delete this 1
        added_ids = []
    ...
    def calculate_missing_features(self, database_index=0, batch_size=None):
        time_series_db = self._data_model_interface.get_time_series_database(database_index)
    ...
    def reduce_all(self, serialization_db_index=0, reduction_model_index=0,
        ...
        time_series_db = self._data_model_interface.get_time_series_database(time_series_db_index)
        batch_iterable = time_series_db.get_all(batch_size)
        reduction_model = self._data_model_interface.get_reduction_model(serialization_db_index, reduction_model_index)
        serialization_db = self._data_model_interface.get_serialization_database(serialization_db_index)
        for time_series_list in batch_iterable:
            print("Trying to add {0} time series to reduction model... ".format(len(time_series_list))),
            n_updated = reduction_model.add_many_time_series(time_series_list)
        ...
        for time_series_list in batch_iterable:
            ...
            n_updated = reduction_model.transform_time_series(time_series_list)
            print("DONE")
            if n_updated > 0:
    ...
    def cluster_all(self, time_series_database_index=0,
        ...
        time_series_db = self._data_model_interface.get_time_series_database(time_series_database_index)
        clustering_model = self._data_model_interface.get_clustering_model(serialization_db_index, clustering_model_index)
        serialization_db = self._data_model_interface.get_serialization_database(serialization_db_index)
